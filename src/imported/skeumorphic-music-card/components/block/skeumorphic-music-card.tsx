...
const ControlButton = ({
  ...
  <div
    className="rounded-full p-[3px] border-[1px] border-[var(--bg)]/5"
    style={{
  ...
  >
    <button
      ...
      className={`rounded-full transition-all duration-75 text-[color-mix(in_srgb,var(--fg)_37%,var(--bg))] hover:text-[color-mix(in_srgb,var(--fg)_21%,var(--bg))] ${
        size === 'large' ? 'p-4' : 'p-2'
      }`}
    >
      {children}
...
const SkeumorphicMusicCard = ({ title, artist, cover, className }: SkeumorphicMusicCardProps) => {
  ...
  return (
    <div
      ...
      className={`p-5 rounded-[30px] border-[1px] border-[var(--bg)]/5 ${className}`}
    >
      <div
        style={{
          boxShadow:
        ...
        className="relative w-80 p-6 rounded-[20px] flex flex-col items-center border-[1px] border-[var(--bg)]/5"
      >
        {/* Album Cover */}
        <div
          style={{
        ...
        <div className="mt-6 text-center">
          <h3 className="text-lg font-semibold text-[color-mix(in_srgb,var(--fg)_28%,var(--bg))]">{title}</h3>
          <p className="text-sm text-[color-mix(in_srgb,var(--fg)_45%,var(--bg))]">{artist}</p>
        </div>
