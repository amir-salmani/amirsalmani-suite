...
export function ParallaxGallery({ images = [], bgColor = "var(--bg)", scroller, viewportHeight = "100vh", frameWidth = 500, frameHeight = 600, thumbnailWidth = 190, thumbnailHeight = 260, className = "" }) {
  ...
  return (
    <section ref={sectionRef} className={`relative font-body text-[var(--fg)] ${className}`} style={{ height: `calc(${viewportHeight} * ${Math.max(total, 1)})`, background: bgColor }}>
      <div className="sticky top-0 w-full select-none overflow-hidden" style={{ background: bgColor, height: viewportHeight }}>
        ...
        <div className="absolute top-1/2 left-1/2" style={{ transform: "translate(-50%, -50%)", zIndex: 20 }}>
          <div className="pointer-events-none absolute top-1/2 left-1/2 z-0 -translate-x-1/2 -translate-y-1/2 border border-dashed border-[var(--fg)]/50" style={{ height: frameH + 24, width: frameW + 24 }} />
