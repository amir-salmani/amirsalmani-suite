...
export function DitherCanvas({
  videoSrc = "/cdn/effects/dither-canvas/dither-canvas-video.mp4?v=3",
  imageSrc = "/cdn/effects/dither-canvas/dither-canvas-poster.webp?v=3",
  className,
  style,
  paused = false,
} = {}) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      actx.fillStyle = tokenColour("--fg", "#fff");
  ...
  return (
    <div className={cn("relative h-[28rem] w-full overflow-hidden bg-[var(--fg)]", className)} style={style}>
      <div
        role="img"
        aria-label="Blue and cyan dither texture on white"
        className="pointer-events-none absolute inset-0"
        style={{
          backgroundImage: "linear-gradient(135deg, transparent 12%, color-mix(in srgb, var(--fg) 85%, transparent) 35%, color-mix(in srgb, var(--fg) 55%, transparent) 55%, color-mix(in srgb, var(--fg) 30%, transparent) 72%, transparent 90%)",
