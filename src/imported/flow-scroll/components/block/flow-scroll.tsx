    ...
    return (
        <motion.div
            style={{
                ...
                boxShadow: useTransform(shadowY, (value) => `0px ${value}px 40px 10px color-mix(in srgb, var(--fg) 10%, transparent)`)
            }}
            className="w-full sm:max-w-48 md:max-w-60 h-32 sm:h-60 md:h-72 overflow-hidden rounded-2xl"
        >
            <img src={image} alt={image} width={1000} height={1000} className="h-full w-full object-cover" />
        </motion.div>
    );
};
