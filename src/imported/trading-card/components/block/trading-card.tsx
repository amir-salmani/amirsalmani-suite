...
const TradingCard: React.FC<TradingCardProps> = ({ imageUrl, rank, name, description }) => {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col hover:scale-105 transition-all duration-200 ease-linear items-start justify-end rounded-lg relative shadow-xl overflow-hidden h-[400px] w-[300px] cursor-pointer border-[1px] border-[color-mix(in_srgb,var(--fg)_28%,var(--bg))]"
    >
      <motion.div
      ...
      >
        ...
        <p className="text-sm text-[var(--fg)]/90">{description}</p>
      </motion.div>
    </motion.div>
  );
};
