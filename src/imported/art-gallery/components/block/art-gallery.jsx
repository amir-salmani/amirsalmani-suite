...
function blankTexture() {
  ...
  const ctx = canvas.getContext("2d");
  if (ctx) {
    ctx.fillStyle = tokenColour("--bg", "#111");
...
function ArtGalleryScene({ images, items, cellSize, zoomLevel, showHint, reducedMotion }) {
  ...
  return (
    <div className="absolute inset-0 cursor-grab active:cursor-grabbing" style={{ touchAction: "none" }}>
      ...
      {!ready ? (
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-[var(--bg)] text-[var(--fg)]" role="status" aria-live="polite">
          <RipplePulseLoader size={150} />
        </div>
      ) : null}
      {ready && showHint ? (
        <div className="pointer-events-none absolute bottom-6 left-1/2 z-10 -translate-x-1/2 font-mono text-[0.6rem] uppercase tracking-[0.2em] text-[var(--fg)]/20">
          drag to explore
        </div>
      ) : null}
    </div>
...
export function ArtGallery({
  ...
  return (
    <WebGLSurface className={cn("bg-[var(--bg)]", className)} style={style} label="ObsidianUI Art Gallery">
      <ArtGalleryScene
        images={tiles}
        items={captions}
        cellSize={cellSize}
