...
export function ButterflyTrailCursor({ className, height = 400, style, text = "ObsidianUI, in flight." } = {}) {
  ...
  return (
    <div className={cn("relative isolate w-full overflow-hidden bg-[var(--bg)]", className)} style={{ height, containerType: "inline-size", ...style }}>
      <Canvas frameloop={motionEnabled ? "always" : "demand"} camera={{ position: [0, 0, 5], fov: 75 }} gl={{ antialias: false, powerPreference: 'high-performance', alpha: false }} dpr={[1, 1]} performance={{ min: 0.5 }}>
        <color attach="background" args={[tokenColour("--bg", "#EAEAE9")]} />
        <Suspense fallback={null}><ButterflyTrail motionEnabled={motionEnabled} /></Suspense>
      </Canvas>
      <p className="pointer-events-none absolute left-1/2 top-1/2 w-full -translate-x-1/2 -translate-y-1/2 text-center font-serif text-[clamp(22px,4cqw,44px)] text-[color-mix(in_srgb,var(--fg)_45%,var(--bg))]">
        {text}
      </p>
    </div>
  )
