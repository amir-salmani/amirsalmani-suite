...
const STYLES: Record<string, Record<string, string>> = {
    trigger: {
        container:
            'rounded-full flex items-center bg-[var(--bg-alt)] justify-center border border-[var(--fg)]/10 cursor-pointer outline-none ring-0 hover:brightness-125 transition-all duration-100 z-50',
        active: 'bg-[var(--bg-alt)]'
    },
    item: {
        container:
            'rounded-full flex items-center justify-center absolute bg-[color-mix(in_srgb,var(--fg)_97%,var(--bg))] text-[var(--bg)] hover:bg-[var(--fg)] cursor-pointer',
        label: 'text-xs text-[var(--bg)] absolute top-full left-1/2 -translate-x-1/2 text-[var(--fg)] mt-1'
    }
};
    ...
    const closeAnimation = async () => {
        ...
        for (let i = 0; i < scaleTransition.length; i++) {
            await animate.start({
                ...
                backgroundColor: `color-mix(in srgb, var(--bg-alt) ${Math.max(100 - i * 10, 40)}%, var(--fg))`,
                transition: {
...
export function CircleMenu({
    items,
    openIcon = <Menu size={18} className="text-[var(--fg)]" />,
    closeIcon = <X size={18} className="text-[var(--fg)]" />
}: CircleMenuProps) {
